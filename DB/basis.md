...
関係のタプルを一意に識別できる属性または属性の組のうち、極小のものを指す。
...
さらに、候補キーは一つとは限らない。上記の関係の場合、「商品番号に対応する商品名の重複はない」という条件があるため、商品名でも、商品番号の代わりに商品を一意に識別できる。つまり、{伝票番号, 商品名}も候補キーの一つとなる。
...
この関係は、「顧客名(顧客番号)」の列に複数の値が含まれており、第一正規形の条件を満たしていないため、非正規系となる。
第一正規系にするには、各列が単一の値を持つ必要があり、以下のようになる。

|<u>伝票番号</u>|顧客名|顧客番号|<u>商品番号</u>|商品名|数量|
|--|--|--|--|--|--|
|1001|ねこ商事|11|2001|チョコレート|5|
|1002|うさぎ開発|13|2002|キャラメル|3|
|1003|くま工業|29|2003|ドーナツ|10|
...
第二正規形は、テーブルが第一正規形を満たしている上で、全ての非キー属性が各候補キーに依存している関係である。
言い換えれば、テーブル内のすべての列が主キーに完全に関連している必要がある。

第一正規形で述べた関係"伝票"について、
{顧客名, 顧客番号}は候補キー{伝票番号, 商品番号}の一部である{伝票番号}に対してのみ依存 (関数従属) している。
候補キーの一部に対して一意に値が定まるような関係を部分関数従属と呼ぶ。
第二正規形では部分関数従属を排除する。
排除するには、部分関数従属を取り出して別のテーブルとして表現する。

部分関数従属: {伝票番号}→{顧客番号, 顧客名}
...
第三正規形は、テーブルが第二正規形を満たしている上で、非キー列間に推移的な依存関係がない関係である。つまり、非キー列同士の依存関係がなく、それぞれの列が直接主キーに依存している状態を指す。
...
という関係を推移的関数従属と呼ぶ。
第二正規形を第三正規形にするには、推移的関数従属性を排除する。
...
例えば、直性の例で使用した二つの関係RとSは、直積R×Sから、関係RのA列と、関係SのA列の値が等しい行を選択し、更に関係SのA列を除いた三つの列の射影を取ることで、次のような関係となる。
